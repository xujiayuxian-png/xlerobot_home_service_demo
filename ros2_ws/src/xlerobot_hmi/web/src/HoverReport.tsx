...
export function HoverReport() {
  ...
  if (!report) return <section className="engineering-card"><h2>悬停验证结果</h2>
    <p>{error || '尚无三点验证结果。点击自动运行，机器人会依次测量三个点并生成建议。'}</p></section>
  ...
  return <section className="engineering-card hover-report">
    <div className="hover-report-heading"><div><p className="eyebrow">HOVER · RESULT</p><h2>悬停验证结果</h2></div>
      <strong>{report.status === 'COMPLETED' ? '测量完成 · 补偿尚未验证' : report.status === 'RUNNING' ? '自动测量中' : '测量中断 · 已完成数据保留'}</strong></div>
    <p>{report.message}</p>
    {data?.stale && <p className="notice" role="alert">标定版本已变化：以下是历史结果，建议补偿不可沿用，请重新测量。</p>}
    {error && <p role="alert">{error}；当前显示缓存结果。</p>}
    <div className="hover-summary-grid">
      <div><small>独立到位</small><strong>{report.independent_arrivals} / 3</strong><span>{report.frames} 帧，不等于 {report.frames} 次重复实验</span></div>
      <div><small>平均平面偏差</small><strong>{report.summary ? number(report.summary.mean_planar_error_mm) : '—'} <em>mm</em></strong></div>
      <div><small>平均高度不足（正值偏低）</small><strong>{report.summary ? number(report.summary.mean_height_shortfall_mm) : '—'} <em>mm</em></strong></div>
    </div>
    {report.arrivals.length > 0 && <>
      <div className="hover-charts">
        <figure><figcaption>俯视对齐 · ○ 目标 / ● 实测（板坐标，mm）</figcaption>
          <svg viewBox="0 0 400 190" role="img" aria-label="三个悬停点的目标与实际平面位置">
            ...
            {report.arrivals.map(row => <g key={row.target}>
              ...
              <text x={200 + row.target_xyz_mm[0] * 3} y="160" textAnchor="middle">{names[row.target]}</text>
            </g>)}
          </svg></figure>
        <figure><figcaption>侧视高度 · 虚线为目标 200 mm</figcaption>
          <svg viewBox="0 0 400 190" role="img" aria-label="三个点的目标高度与实测高度">
            ...
            {report.arrivals.map((row, i) => <g key={row.target}>
              ...
              <text x={90 + i * 110} y="181" textAnchor="middle">{names[row.target]}</text>
            </g>)}
          </svg></figure>
      </div>
      <table><thead><tr><th>点位</th><th>平面偏差</th><th>实际高度</th><th>高度不足</th><th>最大关节到位偏差</th></tr></thead><tbody>
        {report.arrivals.map(row => <tr key={row.target}><td>{names[row.target]}</td><td>{number(row.planar_error_mm)} mm</td>
          <td>{number(row.height_mm)} mm</td><td>{number(row.height_shortfall_mm)} mm</td><td>{number(row.max_joint_error_deg)}°</td></tr>)}
      </tbody></table>
    </>}
    <div className="hover-suggestion"><h3>建议补偿 · 仅作为下一轮验证初值</h3>
      {usable && suggestion ? <>
        <p>建议目标高度{suggestion.raise_target_mm >= 0 ? '增加' : '降低'} <strong>{number(Math.abs(suggestion.raise_target_mm))} mm</strong>。
          板 X 偏移 <strong>{(suggestion.add_to_target_xyz_m[0] * 1000).toFixed(1)} mm</strong>，
          板 Y 偏移 <strong>{(suggestion.add_to_target_xyz_m[1] * 1000).toFixed(1)} mm</strong>。</p>
        <pre>{`frame: calibration_board\nadd_to_target_xyz_m: [${suggestion.add_to_target_xyz_m.map(v => v.toFixed(5)).join(', ')}]\napplied: false`}</pre>
        <p>使用方式：新目标 = 原目标 + 上述偏移。板 Z 朝下，因此负 Z 表示抬高；不是机器人 base_link 的 XYZ 参数。</p>
        <p>{suggestion.advice}</p>
        <p>三点之间的偏差标准差（X / Y / Z）：{suggestion.between_pose_error_std_mm.map(number).join(' / ')} mm。
          这是姿态间变化，不是补偿后的实测精度；常量偏移可能无法消除全部误差。</p>
        <button onClick={download}>下载建议参数（不应用）</button>
      </> : <p>{data?.stale ? '历史建议已失效。' : '完成同一标定版本、同一桌面场景的三点测量后生成建议。'}</p>}
    </div>
    <p>这是同相机视觉计量，不是独立尺量真值。不把总误差直接归因为重力下垂，不改写舵机零位、手眼矩阵或 Demo 生效配置。</p>
    <small>测量记录：{report.id} · {report.updated_at}{report.source === 'imported_previous_run' ? ' · 来自上轮已完成的三点实测' : ''}</small>
  </section>
}

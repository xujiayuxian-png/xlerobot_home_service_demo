...
ServoCalibrationSession::ServoCalibrationSession(std::string unit_id, std::string bus_identity)
: unit_id_(std::move(unit_id)), bus_identity_(std::move(bus_identity)), captures_(specs().size())
{
...
std::vector<size_t> ServoCalibrationSession::group(const std::string & name) const
{
  ...
  for (size_t i = 0; i < specs().size(); ++i) {
    if (specs()[i].group == name) {indices.push_back(i);}
  }
  if (indices.empty()) {throw std::runtime_error("group must be right_arm, left_arm, or head");}
  return indices;
}
...
void ServoCalibrationSession::start_range(const std::string & name)
{
  ...
  for (const auto i : group(name)) {
    ...
    if (!row.fault.empty()) {throw std::runtime_error(specs()[i].key() + ": " + row.fault);}
    if (!row.online || !row.position) {throw std::runtime_error(specs()[i].key() + ": read failed");}
  }
  active_group_ = name;
...
void ServoCalibrationSession::restore(const std::filesystem::path & file)
{
  ...
  if (node["schema"].as<std::string>() != "xlerobot_servo_session/v1" ||
    ...
    !node["joints"].IsMap() || node["joints"].size() != specs().size())
  {
...
void ServoCalibrationSession::validate_complete() const
{
  ...
  for (size_t i = 0; i < specs().size(); ++i) {
    ...
    if (!row.range_captured || !row.zero || !row.raw_min || !row.raw_max || !row.fault.empty() ||
      *row.zero < *row.raw_min || *row.zero > *row.raw_max || coverage(i) < kMinimumCoverage)
    {
      throw std::runtime_error("calibration is incomplete: " + specs()[i].key());
    }
  }
}

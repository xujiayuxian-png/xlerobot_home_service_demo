...
Run `./tools/doctor robot` to identify missing setup. Follow the
[calibration guide](calibration.md) for supervised measurements; normal demo
motion requires the completed active bundle. Stay beside the robot for motion,
keep a physical stop method available, and support an arm before releasing torque.

After calibration, follow [mapping and named places](mapping.md), then
[demo startup](demo.md). Each live tool needs the literal `--hardware` flag.

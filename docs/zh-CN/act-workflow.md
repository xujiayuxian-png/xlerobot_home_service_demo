...
它会启动源码采集工作区，右侧 Follower 机械臂可能运动。通过 Web UI 管理每条不可变
episode；原始录制不进入 Git。
...
6. 新采样正常保存并通过数据完整性检查后**默认保留**，无需点击通过。不想用于训练时
   点 **拒绝本条**，误拒绝可 **恢复保留**；均不删除或修改原始文件。最近 20 条已完成
   采样在开始下一条、刷新或重启页面后仍可改选。旧的未审核数据不自动纳入，可手动保留。
   自动保留记录 `selection_source: automatic_on_save`，不冒充人工审核。

...
随动输出则使用当前 active 从臂标定中的运动限位。启动不自动回位或开始录制。

1. 首次试录选择 **通用手动**，时长设为 10–15 秒。它以从臂当前姿态为起点，
   不进行目标检测或自动预抓取；**抓取模板** 则先识别目标、把从臂移到目标上方，
   复用 Demo 的预抓取流程。
2. 确认数据集 ID，填写实际物体和任务描述，如“黄色胶棒 / 抓住黄色胶棒”。
   试录使用独立数据集，不混入正式示教。
3. 点击 **Home / 开始采集**。两个模式都会自动把主臂对齐从臂；这时不要拖动主臂。
   倒计时阶段准备接住主臂，等显示 `RECORDING` 后再拖动，右侧从臂随动。
4. 完成后点击 **End / 正常结束并发布**；达到最长时长也会正常结束。系统先停止
   随动，再保存双相机视频和关节数据。这里“发布”仅表示本地完成 episode，不会上传。
5. 等待保存成功，选择 **接受** 或 **拒绝**。只有接受的 episode 进入转换；拒绝
   不删除原始文件。再次点击 Home 会开始新的一条，不覆盖上一条。

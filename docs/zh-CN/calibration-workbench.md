...
右上角 **EN / 中文** 按钮可以切换工作台界面和状态提示，选择会保存在浏览器中。
标定路径、版本 ID、测量值和设备名称保持原样，不会被翻译。
确认弹窗、图表标签和本地文档链接也会同步切换；切换不重建控制页面、不重启画面，
不发送设备命令。常见服务信息提供翻译，未知诊断和会话日志保留原文便于排查；
英文界面会将未知中文诊断标注为 `Original service detail`。

维护方式：组件使用 `useLanguage().t()` 显式渲染文案，词条位于
`xlerobot_hmi/web/src/translations.ts`。异步操作提示用 `msg(source, ...values)`
保存；用户数据只作为参数传入，不参与翻译。`serviceTranslations.ts` 只匹配已知的
完整服务消息，不改变 ROS 协议。修改文案后，在 `ros2_ws/src/xlerobot_hmi/web`
运行 `npm test` 和 `npm run build`；测试覆盖词条完整性、英文页面、双向切换、
数采提交内容不变及遥控连接不中断。

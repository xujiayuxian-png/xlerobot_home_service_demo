...
先运行 `./tools/doctor robot` 确认缺失项，再按[标定说明](calibration.md)
在现场监护下测量；正常 Demo 运动需要完整激活的标定。
运动时留在机器人旁，保留物理停止手段，释放扭矩前托住机械臂。

标定后继续[建图与地点](mapping.md)，再[启动 Demo](demo.md)。
所有真机入口都需要字面参数 `--hardware`。

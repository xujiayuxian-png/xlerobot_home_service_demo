...
两条命令分阶段运行，不要同时启动；使用自定义 `--config` 时两阶段必须相同。
建图需要重来时，先结束遥控并停稳，再点“清除当前地图并重建”并确认。
这只重置实时 SLAM，不删除已保存的地图、地点或 Demo 配置；未保存的建图结果无法恢复。
雷达会继续生成新地图。重建后请重新确认或记录地点坐标，再保存新地图。
网页中的顺序是：

1. 开启遥控，按住摇杆覆盖场地。上/下前后行驶、左/右转向，可同时转弯；
   松手回零，切走窗口会结束遥控。速度滑块设置的是最大速度。
2. 到目标位置后结束遥控，等机器人停稳，再记录地点。`table` 是最终桌边停车位，
   朝向应面向桌面；导航时先在后方 0.25 m 处到达，再精确贴桌。
   已记录地点显示在列表和地图上；点击列表可选择更新或删除。
3. 全部场地覆盖完成后保存当前地图。保存后仍可继续建图，但结束前需要再次保存。
   页面显示保存时间；草稿不等于已启用的 Demo 地图。
4. 停止 build，再启动 validate。先自动定位，再逐地点导航验证，最后激活草稿。
   重存地图会清除旧地图的验证结果；更新某个地点会清除该地点的验证结果。
5. 激活后的文件位于 `<data.collection_root>/sites/<robot.site_id>/current/`。
   在本机配置中将 `site.map` 和 `site.places` 分别指向其中的 `map.yaml` 与
   `places.yaml`，下次启动 Demo 才会使用它们。测试场地不会自动覆盖旧场地配置。
